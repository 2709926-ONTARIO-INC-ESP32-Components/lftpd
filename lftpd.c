...
static int cmd_pass(lftpd_client_t* client, const char* arg) {
	...
	if (arg != NULL && strcmp(arg, VALID_PASSWORD) == 0) {
...
static int cmd_user(lftpd_client_t *client, const char *arg)
{
    ...
    if (strcmp(arg, VALID_USERNAME) == 0)
    {
...
static int handle_control_channel(lftpd_client_t *client)
{
	...
	if (read_buffer == NULL)
	{
		lftpd_log_error("failed to allocate read_buffer");
		err = -1;
		goto cleanup;
	}
	...
	while (err == 0)
	{
		...
		{
			size_t expected_len = strlen(commands[i].command);
			if (strncmp(commands[i].command, command_tmp, expected_len) == 0 &&
				strlen(command_tmp) == expected_len)
			{
				char *arg = NULL;
